# Generate combined training and validation loss (no-mixup)
...
if __name__ == '__main__':
    ...
    path_rand_seed_sav      = path + "302_Realz_Models/6_RandSeed_Predictions/pinklady_sims/1_m1_NoMixup/" #Save DL models (pinklady)
    #
    ...
    train_loss_mat       = np.ndarray(shape = (epochs,num_random_seeds), dtype = float) #(4000,15)
    val_loss_mat         = np.ndarray(shape = (epochs,num_random_seeds), dtype = float) #(4000,15)
    #
    for i in range(0,num_random_seeds):
    ...
    df_train_loss_mat = pd.DataFrame(train_loss_mat, index = [str(i) for i in range(0,epochs)], \
                                    columns = [str(random_seed_list[i]) for i in range(0,num_random_seeds)]) #[4000 rows x 15 columns]
    df_val_loss_mat   = pd.DataFrame(val_loss_mat, index = [str(i) for i in range(0,epochs)], \
                                    columns = [str(random_seed_list[i]) for i in range(0,num_random_seeds)]) #[4000 rows x 15 columns]
    #
    train_loss_lb     = np.amin(train_loss_mat, axis = 1) #(4000,)
    train_loss_ub     = np.amax(train_loss_mat, axis = 1) #(4000,)
    val_loss_lb       = np.amin(val_loss_mat, axis = 1) #(4000,)
    val_loss_ub       = np.amax(val_loss_mat, axis = 1) #(4000,)
