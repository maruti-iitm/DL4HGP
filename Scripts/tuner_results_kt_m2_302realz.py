# RESULTS of KerasTuner -- DL model tuning on 302 REALZ DATA (see tuner_results_kt_m2_160comps.py for 200 realz)
# Paths for NEW data (*.npy files)
#
# Hyperparameter tuning for encoded DNNs (Strategy-m2: No Mixup)
...
if __name__ == '__main__':
    ...
    path_rand_seed_sav      = path + "302_Realz_Models/6_RandSeed_Predictions/2_m2_PreMixup/" #Save DL models
    #
    ...
    #
    if normalization_name == "Not_Normalized": #PCA perm comps unnormalized using Pre-Computed Mixup
        ...
        #
        train_pots_mixup_fl = path_proccessed_data + "4_PCA_Mixup/train_pca_comp_potential_mixup.npy"
        train_perm_mixup_fl = path_proccessed_data + "4_PCA_Mixup/train_pca_comp_permeability_mixup.npy"
        #
    ...
    #
    train_pots_mixup     = np.load(train_pots_mixup_fl) #[396210 rows x 270 columns]
    train_perm_mixup     = np.load(train_perm_mixup_fl) #[396210 rows x 246 columns]
    ...
    i = 1 #see the results of i-th realz
    #
    K.clear_session()
    ...
    directory            = path + "Python_Scripts/DL_Scripts/kt_m2_160comps/" + \
